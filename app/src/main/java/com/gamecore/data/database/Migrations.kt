...
internal object GameCoreMigrations {
    ...
    /**
     * Version 11 → 12: the per-profile resolution override (§B) and its session record.
     *
     * Two additive columns, one on each table, and additive in the strictest sense — both are
     * `ALTER TABLE … ADD COLUMN … TEXT` with no default, no table rebuilt, no row rewritten, no index
     * touched. `game_profiles.resolution_override` holds a [com.gamecore.core.model.ResolutionScale]
     * name, NULL meaning "leave the resolution alone", so every profile written before this feature
     * existed reads back with the override off — which is also a new profile's default. `sessions
     * .resolution_applied` is the session record of what was applied, NULL for a session that predates
     * the feature or ran without it. The mappers parse both by name and fall back to null, so a NULL
     * column is read as "not set" rather than a spurious `FULL`.
     *
     * The column names and types match [GameProfileEntity] and [SessionEntity] exactly, so Room's
     * schema validation against `12.json` passes without a rebuild.
     */
    val MIGRATION_11_12 = object : Migration(11, 12) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `game_profiles` ADD COLUMN `resolution_override` TEXT")
            db.execSQL("ALTER TABLE `sessions` ADD COLUMN `resolution_applied` TEXT")
        }
    }

    /**
     * Version 12 → 13: the config-editor backup ledger.
     *
     * One new table, `config_backups`, and nothing touched on any existing one — additive in the
     * same strict sense as the Aim Lab tables were: a `CREATE TABLE` and two `CREATE INDEX`, no drop,
     * no rewrite, no column added to a table that already held rows. A user updating from 12 has no
     * config backups yet — that is the truth — so the table starts empty and there is nothing to
     * back-fill.
     *
     * The table is the ledger for §A6's "first edit preserves the original" rule: it records that a
     * file was copied aside, where the copy is, and enough to warn and restore, never the bytes. The
     * unique index on `backup_path` is load-bearing rather than an optimisation — it is what makes the
     * original's first-write-wins a database invariant, so `insertIfAbsent` on the deterministic
     * original path can never overwrite the one untouched copy. `kind` holds a
     * [com.gamecore.core.model.ConfigBackupKind] name, parsed back defensively like every enum here.
     *
     * The `CREATE TABLE`/`CREATE INDEX` text matches what Room generates for [ConfigBackupEntity],
     * verified against the exported `13.json` after the build — an autogenerated `Long` id is
     * `INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL`, nullable columns omit `NOT NULL`. No
     * `fallbackToDestructiveMigration`, here as everywhere.
     */
        ...
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                "CREATE TABLE IF NOT EXISTS `config_backups` (" +
                    ...
                    "`sha256` TEXT, " +
                    "`source_last_update_time` INTEGER, " +
                    "`created_at` INTEGER NOT NULL)",
            )
            db.execSQL(
                "CREATE UNIQUE INDEX IF NOT EXISTS `index_config_backups_backup_path` " +
                    "ON `config_backups` (`backup_path`)",
            )
            db.execSQL(
                "CREATE INDEX IF NOT EXISTS " +
                    "`index_config_backups_package_name_user_id_relative_path` " +
                    "ON `config_backups` (`package_name`, `user_id`, `relative_path`)",
            )
        }
    }
